...
export class CommonService {
  ...
  public async buscarPorCnpj (cnpj: string, host: string): Promise<any> {
    ...
    const response: any = await this.backend.apiGetExternal(`${prefix}pessoa/buscaPorCnpj?pes_cnpj=${cnpj}&sis_codigo=6&host=${host}`, null, (): string | null => {
      return null;
    });
    if ( response ) {
  ...
  public async buscarPorCpf (cpf: string, host: string): Promise<any> {
    ...
    const response: any = await this.backend.apiGetExternal(`${prefix}pessoa/validarCpf?pes_cpf=${cpf}&sis_codigo=6&host=${host}`, null, (): string | null => {
      return null;
    });
    if ( response ) {
