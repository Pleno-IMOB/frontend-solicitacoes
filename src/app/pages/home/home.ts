...
export class Home implements OnInit {
  ...
  async ngOnInit (): Promise<void> {
    ...
    this.convertToBlobUrl(`${this.backend.hostAPI}vistoria/logo-imobiliaria?host=${this.backend.urlSistema}`)
      .then(blobUrl => {
        this.logoEmpresa = blobUrl;
      });
  }
  ...
  protected async finalizarAgendamento (): Promise<void> {
    while ( this.backend.isHostAberto ? (!this.authService.pessoa.value?.pes_codigo) : (!this.authService.pessoa.value?.cli_codigo || !this.authService.pessoa.value?.pes_codigo) ) {
      await this.login();
    }
    ...
    if ( response ) {
      ...
      UtilsService.notifySuccess(
        this.backend.isHostAberto ? 'Requisição confirmada.' : 'Sua solicitação de vistoria foi agendada com sucesso.',
        this.backend.isHostAberto ? 'Sucesso!' : 'Agendado!'
      );
      this.thread_id = '';
  ...
  protected async meusDados (): Promise<void> {
    const dialog: MatDialogRef<MeusDados> = this.dialog.open(MeusDados, {
      data: {},
      width: '80vw',
      maxWidth: '500px',
      maxHeight: '90vh',
  ...
  protected async login (): Promise<void> {
    const dialog: MatDialogRef<Login> = this.dialog.open(Login, {
      data: {},
      width: '80vw',
      maxWidth: '500px',
      maxHeight: '90vh',
  ...
  /**
   * Converte uma URL de imagem em um Blob URL.
   * @param imageUrl URL da imagem a ser convertida
   * @returns Uma promessa que resolve para um Blob URL
   */
  private async convertToBlobUrl (imageUrl: string): Promise<string> {
